...
export default function TeamPage() {
  ...
  async function load() {
    ...
    const { data: teamData } = await supabase
      ...
      .select('*')
      .or(`team_id.eq.${teamIdValue},team_name.eq.${teamIdValue}`)
      .maybeSingle()

    setTeam(teamData)

    const resolvedTeamId = teamData?.team_id
    ...
    if (currentSeasonId) await loadChampionsLeague(resolvedTeamId, currentSeasonId)
    ...
    const { data: projectedData } = await supabase
      .from('active_bundesliga_projected_prize_money')
      .select('*')
      .eq('team_id', resolvedTeamId)
      .maybeSingle()
    ...
    const { data: accountData } = await supabase
      .from('active_team_finance_account')
      .select('account_id, user_id')
      .eq('team_id', resolvedTeamId)
      .maybeSingle()

    if (accountData?.account_id) {
      setFinanceAccountId(accountData.account_id)
  ...
  async function loadFinanceTransactions(accountId: string) {
    ...
    if (financeSeasonFilter !== 'all') query = query.eq('season_id', financeSeasonFilter)
  ...
  const displayStats = {
    ...
    tip_points_for: team?.tip_points_for ?? 0,
    tip_points_against: team?.tip_points_against ?? 0,
  }
      ...
      <main className="page-shell" style={{ maxWidth: 1150 }}>
        {!team ? (
          <section className="card" style={{ padding: 24 }}>
            Dieses Team wurde nicht gefunden.
          </section>
        ) : (
          <>
            <section style={heroStyle}>
              ...
              <div style={teamHeroContentStyle}>
                <div style={teamIdentityStyle}>
                  {displayStats.logo_url && (
                    <div style={logoWrapStyle}>
                      <img src={displayStats.logo_url} alt="" style={heroLogoStyle} />
                    </div>
                  )}

                  <div>
                    <div style={eyebrowStyle}>Teamprofil</div>
                    <h1 style={teamTitleStyle}>{displayStats.team_name}</h1>
                ...
                <Link
                  href={`/team/${displayStats.team_id}/sondertipps`}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <text x={8} y={39} fontSize={20} fontWeight="900" fill="#16a34a">S</text>
        <text x={8} y={99} fontSize={20} fontWeight="900" fill="#ca8a04">U</text>
        <text x={8} y={159} fontSize={20} fontWeight="900" fill="#dc2626">N</text>
